 ...
 * @returns {string[]} - An array of filenames with the specified extension in the folder.
 * @throws {Error} - If there is an error reading the folder or if files have different extensions.
 */
export function extractFilenamesFromFolder(relativePathFromPublic) {
  const absoluteFolderPath = path.join(process.cwd(), `public/${relativePathFromPublic}`);

  try {
    const filenames = readdirSync(absoluteFolderPath);
